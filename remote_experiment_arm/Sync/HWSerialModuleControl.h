...
public:
    HWSerialModuleControl(SoftwareSerial& A, uint8_t moduleID)
        : LobotSerialServoControl(A)
        , moduleID(moduleID) { };
    HWSerialModuleControl(SoftwareSerial& A, int receiveEnablePin, int transmitEnablePin, uint8_t moduleID)
        : LobotSerialServoControl(A, receiveEnablePin, transmitEnablePin)
        , moduleID(moduleID) { };
    HWSerialModuleControl(HardwareSerial& A, uint8_t moduleID)
        : LobotSerialServoControl(A)
        , moduleID(moduleID) { };
    HWSerialModuleControl(HardwareSerial& A, int receiveEnablePin, int transmitEnablePin, uint8_t moduleID)
        : LobotSerialServoControl(A, receiveEnablePin, transmitEnablePin)
        , moduleID(moduleID) { };
